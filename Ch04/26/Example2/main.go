...
func main() {
	...
	go func(ch chan int, wg *sync.WaitGroup) {
		...
		close(ch)
		// try to receive new message from close message will Zero-value
		// Zero-value is default value for channel's type, in this case channel for integer messages so zero-value is zero
		fmt.Println(<- ch)
		wg.Done()
	} (channel, waitGroup)
